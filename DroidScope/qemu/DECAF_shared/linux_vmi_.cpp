...
static void zygote_init_callback(DECAF_Callback_Params* params)
{
	...
	char second_name[VMI_MAX_PROCESS_NAME_FULL+1];
	...
	target_ulong mm, mm_arg, name_ptr;
    ...
    for (int count = MAX_LOOP_COUNT; count > 0; --count)
    {
				...
				//if (!VMI_find_process_by_pid(task_pid))
        {
            ...
            BREAK_IF(DECAF_read_mem(env, next_task + OFFSET_PROFILE.ts_comm,
						...
						if(strstr(pe->comm_name, "zygote") != NULL)
						{
							...
							if(zygote_init_cb_handle == DECAF_NULL_HANDLE)
							{
								...
								zygote_init_cb_handle = DECAF_registerOptimizedBlockEndCallback(&zygote_init_callback ,INV_ADDR ,zygote_init_addr);
							}
						...
						//monitor_printf(default_mon,"process %s started pid [%08x]\n", pe->name, pe->pid);
